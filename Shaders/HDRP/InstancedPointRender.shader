Shader "Custom/HDRP/InstancedPointRender"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Unlike Unity's own unity_ObjectToWorld, this matrix is plain
            // Transform.localToWorldMatrix — i.e. an absolute-world-space
            // matrix HDRP knows nothing about, so the position it produces
            // has to be explicitly rebased below (GetCameraRelativePositionWS)
            // before going into TransformWorldToHClip, which under HDRP's
            // (default-on) camera-relative rendering expects camera-relative
            // positions, not absolute ones.
            ...
            Varyings vert(Attributes IN)
            {
                ...
                // Rebase the absolute world position _ObjectToWorld produced
                // into HDRP's camera-relative space (a no-op when camera-
                // relative rendering is disabled) before it reaches
                ...
                Varyings OUT;
                OUT.positionHCS = TransformWorldToHClip(positionRWS);
                OUT.color = lutColor;
